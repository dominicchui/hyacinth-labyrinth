...
VKSwapChain::VKSwapChain(VKDeviceManager& deviceRef, VkExtent2D extent)
    : m_device(deviceRef), windowExtent(extent) {
  init();
}
...
VKSwapChain::VKSwapChain(
  ...
  ) : m_device(deviceRef), windowExtent(extent), oldSwapChain(previous)
{
  init();
...
void VKSwapChain::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        swapChainImageViews[i] =
            m_device.createImageView(swapChainImages[i], swapChainImageFormat);
    }
}

void VKSwapChain::createShadowMapRenderPass() {
    ...
    depthAttachmentRef.attachment = 0;  // One and only attachment
...
void VKSwapChain::createShadowFramebuffers() {
    shadowMapFramebuffers.resize(imageCount());
    for (size_t i = 0; i < imageCount(); i++) {
        std::array<VkImageView, 1> attachments = {m_device.shadowImageViews[i]};
...
void VKSwapChain::createShadowResources() {
    VkFormat depthFormat = findDepthFormat();
    swapChainDepthFormat = depthFormat;

    shadowImages.resize(imageCount());
    shadowImageMemorys.resize(imageCount());
    m_device.shadowImageViews.resize(imageCount());
    ...
    for (int i = 0; i < shadowImages.size(); i++) {
        ...
        if (vkCreateImageView(m_device.device(), &viewInfo, nullptr, &m_device.shadowImageViews[i]) != VK_SUCCESS) {
            throw std::runtime_error("failed to create texture image view!");
        }
    }
}
