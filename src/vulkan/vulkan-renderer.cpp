...
void VKRenderer::beginShadowMapRenderPass(VkCommandBuffer commandBuffer) {
    assert(isFrameStarted && "Can't call beginShadowmMapRenderPass if frame is not in progress");
    assert(
        commandBuffer == getCurrentCommandBuffer() &&
        "Can't begin render pass on command buffer from a different frame");
    ...
    renderPassInfo.framebuffer = m_swapChain->getShadowFrameBuffer(currentImageIndex);

    renderPassInfo.renderArea.offset = {0, 0};
    renderPassInfo.renderArea.extent = m_swapChain->getShadowSwapChainExtent();
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = static_cast<float>(m_swapChain->getShadowSwapChainExtent().width);
    viewport.height = static_cast<float>(m_swapChain->getShadowSwapChainExtent().height);
    viewport.minDepth = 0.0f;
    viewport.maxDepth = 1.0f;
    VkRect2D scissor{{0, 0}, m_swapChain->getShadowSwapChainExtent()};
    vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
}
