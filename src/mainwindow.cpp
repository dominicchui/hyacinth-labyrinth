...
void MainWindow::initialize() {
    ...
    // Create checkbox for invert filter
    filter1 = new QCheckBox();
    filter1->setText(QStringLiteral("Invert Filter"));
    filter1->setChecked(false);

    // Create checkbox for grayscale filter
    filter2 = new QCheckBox();
    filter2->setText(QStringLiteral("Grayscale Filter"));
    filter2->setChecked(false);

    // Create checkbox for blur filter
    filter3 = new QCheckBox();
    filter3->setText(QStringLiteral("5x5 Blur Filter"));
    filter3->setChecked(false);

    // Create checkbox for sobel filter
    filter4 = new QCheckBox();
    filter4->setText(QStringLiteral("Edge Filter"));
...
void MainWindow::connectInvertFilter() {
    connect(filter1, &QCheckBox::clicked, this, &MainWindow::onInvertFilter);
}

void MainWindow::connectGrayscaleFilter() {
    connect(filter2, &QCheckBox::clicked, this, &MainWindow::onGrayscaleFilter);
}

void MainWindow::connectBlurFilter() {
    connect(filter3, &QCheckBox::clicked, this, &MainWindow::onBlurFilter);
}

void MainWindow::connectSobelFilter() {
    connect(filter4, &QCheckBox::clicked, this, &MainWindow::onSobelFilter);
}
...
void MainWindow::onInvertFilter() {
    settings.invertFilter = filter1->isChecked();
    realtime->settingsChanged();
}

void MainWindow::onGrayscaleFilter() {
    settings.grayscaleFilter = filter2->isChecked();
    realtime->settingsChanged();
}

void MainWindow::onBlurFilter() {
    settings.blurFilter = filter3->isChecked();
    realtime->settingsChanged();
}

void MainWindow::onSobelFilter() {
    settings.sobelFilter = filter4->isChecked();
    realtime->settingsChanged();
}
...
void MainWindow::onUploadFile() {
    ...
    realtime->sceneChanged(metaData);
}
