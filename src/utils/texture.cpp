    ...
    height(0)
{
    ...
    for (int i = 0; i < arr.size() / 4.f; i++){
        data.push_back({(std::uint8_t) arr[4*i], (std::uint8_t) arr[4*i+1], (std::uint8_t) arr[4*i+2], (std::uint8_t) arr[4*i+3]});
    }
}

Texture::~Texture() {}

glm::vec4* Texture::getData(void) {
    return data.data();
}

glm::vec4 Texture::at(float u, float v) {
    // UV -> XY
