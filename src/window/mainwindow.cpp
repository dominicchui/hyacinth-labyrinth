...
void MainWindow::initialize() {
    ...
    filter1->setText(QStringLiteral("Invert Filter"));
    filter1->setChecked(false);

    // Create checkbox for kernel-based filter
    filter2 = new QCheckBox();
    filter2->setText(QStringLiteral("Box Blur Filter"));
    ...
    // Extra Credit:
    ec1 = new QCheckBox();
    ec1->setText(QStringLiteral("Grayscale Filter"));
    ec1->setChecked(false);

    ec2 = new QCheckBox();
    ec2->setText(QStringLiteral("Sharpen Filter"));
    ec2->setChecked(false);

    ec3 = new QCheckBox();
    ec3->setText(QStringLiteral("Gamma Tone Mapping"));
    ec3->setChecked(false);

    ecG = new QSlider(Qt::Orientation::Horizontal); // Gamma slider
    ecG->setTickInterval(1);
    ecG->setMinimum(1);
    ecG->setMaximum(25);
    ecG->setValue(1);

    ecG = new QSlider(Qt::Orientation::Horizontal); // Far plane slider
    ecG->setTickInterval(1);
    ecG->setMinimum(1);
    ecG->setMaximum(20);
    ecG->setValue(1);
    ecGBox = new QDoubleSpinBox();
    ecGBox->setMinimum(0.1);
    ecGBox->setMaximum(2.0);
    ecGBox->setSingleStep(0.1f);
    ecGBox->setValue(0.1);

    QGroupBox *gammaLayout = new QGroupBox(); // horizonal near slider alignment
    QHBoxLayout *lgamma = new QHBoxLayout();
    lgamma->addWidget(ecG);
    lgamma->addWidget(ecGBox);
    gammaLayout->setLayout(lgamma);

    ec4 = new QCheckBox();
    ec4->setText(QStringLiteral("Laplacian Edge Detect"));
...
void MainWindow::connectExtraCredit() {
    ...
    connect(ecG, &QSlider::valueChanged, this, &MainWindow::onExtraCreditGammaSlider);
    connect(ecGBox, static_cast<void(QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),
            this, &MainWindow::onExtraCreditGammaBox);
...
void MainWindow::onSaveImage() {
    ...
    QString filePath = QFileDialog::getSaveFileName(
        ...
        QDir::currentPath()
            ...
            .append("required")
            .append(QDir::separator())
            .append(QString::fromStdString(sceneName)),
        tr("Image Files (*.png)"));
...
void MainWindow::onExtraCreditGammaSlider(int newValue) {
    ecGBox->setValue(newValue / 10.f);
    settings.ecGamma = ecGBox->value();
    realtime->settingsChanged();
}

void MainWindow::onExtraCreditGammaBox(double newValue) {
    ecG->setValue(newValue * 10.f);
    settings.ecGamma = ecGBox->value();
    realtime->settingsChanged();
}
