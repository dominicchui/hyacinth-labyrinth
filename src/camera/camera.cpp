...
bool Camera::rotate(float delta_x, float delta_y, float deltaTime) {
    ...
    theta_pitch += theta_y;
    if (theta_y > 0.f) {
        if (theta_pitch > theta_pitch_max) {
            float delta = theta_pitch - theta_pitch_max;
            theta_y -= delta;
        }
    } else if (theta_y < 0.f) {
        if (theta_pitch < theta_pitch_min) {
            float delta = theta_pitch - theta_pitch_min;
            theta_y += delta;
        }
    }

    right = glm::cross(-look, up);
    glm::mat3 rot_right = getRotationMatrixRodrigues(theta_y, right);

    look = rot_up * rot_right * look;
...
void Camera::recomputeMatrices() {
    ...
    float t_w = t_h * aspect_ratio;

    proj_mat = glm::perspective(t_h, aspect_ratio, near, far);
    //printMat4(proj_mat);

    float tan_t_w_2 = std::tan(t_w / 2.f);
    float tan_t_h_2 = std::tan(t_h / 2.f);

    glm::mat4 scale_view(
        1.f/(far*tan_t_w_2), 0                  , 0      , 0,
        0                  , 1.f/(far*tan_t_h_2), 0      , 0,
        0                  , 0                  , 1.f/far, 0,
        0                  , 0                  , 0      , 1
        );
    ...
    glm::mat4 m_proj2 = opengl_perspective * unhinge * scale_view;
